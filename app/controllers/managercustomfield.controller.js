...
function checkAlreadyExists(id,sportId,name, orgId)
    {
            ...
            {
                _id = new ObjectID(id);
                var inputjson = {_id: {$ne: _id }, "sport_id":sportId, "field_name": name, "organization_id": orgId}
            }else{
                var inputjson = {"sport_id":sportId, "field_name": name, "organization_id": orgId}
            }

           return Managercustomfield.find(inputjson).then(function(result){
            ...
            if(logger.exitOnError == true){
                ...
                .then(data => {
                    ...
                    if( data._id ){
                        ...
                        Managercustomfield.findByIdAndUpdate(data._id, json, {new: true}, function (err, place) {
                            if(logger.exitOnError == true){
...
exports.findbyOrgAll = (req, res) => {
	...
	Managercustomfield.find({organization_id: orgId})
              .then(manage => {
                if(logger.exitOnError == true)
                {
